"""Combine hard-mode loophole metrics outputs and write aggregate plots."""
...
DEFAULT_OUTPUT_DIR = Path(
    "experiments/results/loophole_learning_metrics_hard_combined"
)
...
def default_result_dirs(results_root: Path, output_dir: Path) -> list[Path]:
    return sorted(
        path
        for path in results_root.glob("loophole_learning_metrics_hard_*")
        if path != output_dir
...
def main() -> None:
    ...
    if not result_dirs:
        raise SystemExit(
            ...
            "'experiments/results/loophole_learning_metrics_hard_*'."
        )
