...
class TaxEnv(gym.Env):
    def __init__(
        ...
        self.cash_amounts = CASH_AMOUNTS or [100.0, 75.0, 50.0, 25.0]
        self.stock_percents = STOCK_PERCENTS or [80.0, 70.0, 50.0, 25.0]
    ...
    def step(self, action):
        ...
        try:
            if action_type == MAKE_SUBCORPORATION:
                ...
                self._contribute_for_stock(
                    issuer_idx=corp_a_idx,
                    contributor_idx=corp_b_idx,
                    stock_idx=stock_idx,
    ...
    def _transfer_cash(self, from_idx: int, to_idx: int, amount_idx: int) -> None:
        ...
        direct_repatriation = from_foreign and to_id == self.state.taxpayer_id
        domestic_child_to_taxpayer = (
            self.state.corporations[from_id].is_domestic
            and to_id == self.state.taxpayer_id
            and self.state.is_subcorporation_of(from_id, to_id)
        )
        if not direct_repatriation and not domestic_child_to_taxpayer:
            raise ValueError("Cash transfer must be direct repatriation or direct-subcorporation cash to taxpayer")
    ...
    def _contribute_for_stock(
        ...
        contributor_id = self._get_corporation_id(contributor_idx)
        stock_id = self._get_stock_id(stock_idx)
        cash_amount = self._get_cash_amount(percent_idx)
        stock_percent = self._get_stock_percent(percent_idx)
        self.state.contribute_for_stock(
            issuer_id=issuer_id,
            contributor_id=contributor_id,
            cash_amount=cash_amount,
            contributed_stock_id=stock_id,
            contributed_stock_percent=stock_percent,
            issued_stock_percent=stock_percent,
        )
        if (
            issuer_id == self.state.taxpayer_id
            and self.state.corporations[contributor_id].is_foreign
        ):
            self.state.record_direct_repatriation(cash_amount)
